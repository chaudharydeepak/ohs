...
public class EhsHelper
{
	...
	public String resetPswdHelper(String email)
	{
		if (manageUserService.userExist(email))
		{
			manageUserService.createPasswordResetTokenForUser(email);
		} else
		{
			return "Unknown User. Check email.";
		}

		return "Please check your email for further instructions to reset password.";
	}
	
	public String processPswdToken ( String username, String token )
	{
		return manageUserService.validatePasswordResetToken( username, token);
	}
}
