...
import com.dc.ehs.common.EHSConstants;
...
public class ApplicationController
{
	...
	public ModelAndView approve(ModelMap model, Observation observation) throws IOException
	{
		...
		ehsHelper.setStatus(Integer.valueOf(observation.getObsID()), EHSConstants.OBS_INPROGRESS_STATUS,
				observation.getActionComments(), observation.getFile());
		return new ModelAndView(EHSConstants.OBS_REDIRECT_ACTION + observation.getObsID());
	}
	...
	public ModelAndView deny(ModelMap model, Observation observation) throws IOException
	{
		...
		ehsHelper.setStatus(Integer.valueOf(observation.getObsID()), EHSConstants.OBS_COMPLETED_STATUS,
				observation.getActionComments(), observation.getFile());
		return new ModelAndView(EHSConstants.OBS_REDIRECT_ACTION + observation.getObsID());
	}
	...
	/**
	 * This action creates new observation in database and redirects traffic to load
	 * the observation.
	 * 
	 * @param observation
	 * @param model
			...
			throws IOException, MessagingException, ParseException
	{
		...
		if (observation.getOperationType().equalsIgnoreCase(EHSConstants.OBS_OPERATE_NEW))
		{
			return new ModelAndView(EHSConstants.OBS_REDIRECT_MGOBS);
		} else
		{
			return new ModelAndView(EHSConstants.OBS_REDIRECT_LOADOBS + obsId);
		}
	}
			...
			throws IOException, MessagingException, ParseException
	{
		...
		/*
		 * make sure observation is not in complete phase, else editing not allowed.
		 */
		String obsStatus = ehsHelper.fetchStatusForObs(Integer.valueOf(observation.getObsID()));

		if (null != obsStatus & !obsStatus.trim().equalsIgnoreCase(EHSConstants.OBS_COMPLETED_STATUS))
		{
			observation.setOperationType(EHSConstants.OBS_OPERATE_EDIT);
			...
			{
				return new ModelAndView(EHSConstants.OBS_REDIRECT_HOME);
			} else
			{
				return new ModelAndView(EHSConstants.OBS_REDIRECT_LOADOBS_ROOT + obsId);
			}
		} else
		{
			return new ModelAndView(EHSConstants.OBS_REDIRECT_LOADOBS_ROOT + observation.getObsID() + "&msg="
					+ EHSConstants.OBS_MSG_NONEDIT);
		}
	}
